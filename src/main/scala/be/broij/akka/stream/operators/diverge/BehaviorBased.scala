...
import be.broij.akka.stream.operators.diverge.BehaviorBased.{Completed, ConsumerLogic, Failed, Register, Registered, Request, Response, Unregister, Unregistered}
...
trait BehaviorBased[T] {
  ...
  protected def preStartIncarnation(incarnation: ConsumerLogic[T]): Unit = synchronized {
    ...
    if (completed) incarnation.completeStage()
      ...
      if (consumerCount == 1 && producer == null) producer = actorSystem.spawnAnonymous(producerBehavior())
  ...
  protected def register(incarnation: ConsumerLogic[T], timeoutDelay: FiniteDuration): Unit = synchronized {
    ...
    producer.ask {
      ...
      incarnation.getAsyncCallback[Try[Response]] {
        ...
        case Success(Completed) => producerCompleted(incarnation)
        case Success(Failed(reason)) => producerFailed(incarnation, reason)
        case Failure(reason) =>
  ...
  protected def postStopIncarnation(incarnation: ConsumerLogic[T]): Unit = synchronized {
    if (consumerCount == 1 && (restartSource || completed || failed.nonEmpty) && producer != null) {
      actorSystem.stop(producer.toClassic)
      producer = null
    }
    consumerCount -= 1
...
object BehaviorBased {
  ...
  case class Failed(exception: Throwable) extends Response
    ...
    def pull(timeoutDelay: FiniteDuration): Unit = {
      ...
      operator.producer.ask(onPullCommand).onComplete(
        getAsyncCallback[Try[Response]] {
          ...
          case Success(Completed) => operator.producerCompleted(this)
          case Success(Failed(reason)) => operator.producerFailed(this, reason)
          case Failure(reason) =>
            if (reason.isInstanceOf[TimeoutException]

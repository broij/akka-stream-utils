...
object Partition {
  /**
    ...
    * waitSlowest is set to false. The created sources complete when the producer completes. They fails when the 
    * producer or any of its consumers fails. The job of the producer is to emit the elements of the wrapped source to 
    * the registered consumers. It manages a dynamic group of consumers that grows or shrinks as consumers register and 
    * unregister. A special flag, restartSource, allows to specify how the producer should react when there is no more 
    * consumers. When the flag is set to true, the producer will stop the wrapped source and restart it from the 
    * beginning when some new consumer register. When set to false, it will let the wrapped source continue to execute. 
    * If an element sent to a consumer isn't ackowledged to the producer before a FiniteDuration defined by 
    * baseTimeoutDelay, the element is sent again to that consumer. This duration is increased exponentially by a power 
    * of two each time the same element is sent again to the same consumer. Note that this mecanism is completely 
    * transparent for the final user: nothing more than providing that baseTimeoutDelay is expected to be done by the 
    * user; when an element is received several time by the same consumer due to retransmissions, it will appear only 

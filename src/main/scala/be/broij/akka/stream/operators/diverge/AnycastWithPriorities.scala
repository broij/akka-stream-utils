...
object AnycastWithPriorities {
  ...
  /**
    ...
    * They fail when the producer fails. The job of the producer is to emit the elements of the wrapped source to the
    * registered consumers. It manages a dynamic group of consumers that grows or shrinks as consumers register and
    ...
    * user; when an element is received several times by the same consumer due to retransmissions, it will
